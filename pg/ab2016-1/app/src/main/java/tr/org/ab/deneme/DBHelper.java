...
/**
 * Created by abdullah on 01/02/16.
 */
public class DBHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<DataModel> getData(){
        ...
        for (int i=0; i<cursor.getCount();i++){
            DataModel data = new DataModel(cursor.getInt(0),cursor.getString(1),cursor.getString(2),cursor.getString(3));
            result.add(data);
            cursor.moveToNext();
        }
        return result;
    ...
    public boolean updateData(int id,String ad,String soyad,String sehir){
        SQLiteDatabase db = this.getWritableDatabase();
        //db.execSQL("UPDATE people SET ad=" + ad +", "+"soyad=" +soyad+", sehir="+sehir + " "+"WHERE id=" +String.valueOf(id));
        ContentValues row = new ContentValues();
        row.put("ad", ad);
        row.put("soyad", soyad);
        row.put("sehir", sehir);
        db.update("people", row,"id=?",new String[]{Integer.toString(id)});
